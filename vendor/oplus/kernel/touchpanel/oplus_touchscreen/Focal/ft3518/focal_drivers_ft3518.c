...
int fts_pinctrl_select_active(struct fts_ts_data *fts3518_data)
{
	int ret = 0;

	if (fts3518_data->hw_res->pinctrl && fts3518_data->hw_res->pin_set_reset_high) {
		ret = pinctrl_select_state(fts3518_data->hw_res->pinctrl, fts3518_data->hw_res->pin_set_reset_high);
		TPD_INFO("%s : ret = %d\n", __func__, ret);
		if (ret < 0) {
			TPD_INFO("Set active pin state error:%d", ret);
			devm_pinctrl_put(fts3518_data->hw_res->pinctrl);
			fts3518_data->hw_res->pinctrl = NULL;
		}
	}
...
int fts_pinctrl_select_suspend(struct fts_ts_data *fts3518_data)
{
	int ret = 0;

	if (fts3518_data->hw_res->pinctrl && fts3518_data->hw_res->pin_set_reset_low) {
		ret = pinctrl_select_state(fts3518_data->hw_res->pinctrl , fts3518_data->hw_res->pin_set_reset_low);
		TPD_INFO("%s : ret = %d\n", __func__, ret);
		if (ret < 0) {
...
{
	if (fts3518_data->need_pinctrl_pull_up_reset) {
		if (on == true) {
			fts_pinctrl_select_active(fts3518_data);
		} else {
			fts_pinctrl_select_suspend(fts3518_data);
		}
	} else {
